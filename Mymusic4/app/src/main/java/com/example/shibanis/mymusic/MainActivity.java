...
public class MainActivity extends ActionBarActivity {
    ...
    private static String url_create_product = "http://74853543.ngrok.com/createproduct.php";
    private static final String TAG_PRODUCTS = "products";
    private static final String TAG_PID = "Name";
    private static final String TAG_NAME = "Tag";
    private static final String TAG_VALUE = "Value";

    private static String url_get_product = "http://74853543.ngrok.com/gettags.php";
  ...
  ArrayAdapter adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (settings.getBoolean("my_first_time7", true)) {
            ...
            db.execSQL("CREATE TABLE song(sid VARCHAR,title VARCHAR,album VARCHAR, artist VARCHAR,year VARCHAR);");
            db.execSQL("CREATE TABLE tagged(sid VARCHAR,tag VARCHAR,value VARCHAR, FOREIGN KEY (sid) references song(sid));");
        ...
        protected Void doInBackground(String... hi) {
            ...
            if(musicCursor!=null && musicCursor.moveToFirst()){
                ...
                do {
                    ...
                    SQLiteDatabase db = openOrCreateDatabase("se_project", MODE_PRIVATE, null);
                    //db.execSQL("DELETE FROM playlist");
                    db.execSQL("INSERT INTO song VALUES('"+String.valueOf(i)+"','"+thisTitle+"','"+thisAlbum+"','"+thisArtist+"','"+thisYear+"');");
                    i++;
                ...
                idColumn = musicCursor.getColumnIndex
                 ...
                 artistColumn = musicCursor.getColumnIndex
                        (android.provider.MediaStore.Audio.Media.ARTIST);
                albumColumn = musicCursor.getColumnIndex
                        (android.provider.MediaStore.Audio.Media.ALBUM);
                yearColumn = musicCursor.getColumnIndex
                        (android.provider.MediaStore.Audio.Media.YEAR);
                SQLiteDatabase db = openOrCreateDatabase("se_project", MODE_PRIVATE, null);
                do {
                    //   JSONParser jsonParser = new JSONParser();
                    ...
                    try {
                        ...
                        if (success == 1) {
                            ...
                            for (int j = 0; j < products.length(); j++) {
                                ...
                                Cursor ci = db.rawQuery("SELECT sid FROM song WHERE title='"+id+"'", null);
                                Set Play = new HashSet();
                                ci.moveToFirst();
                                //Toast.makeText(getApplicationContext(),c.getString(0) + "",Toast.LENGTH_LONG).show();
                                StringBuffer buffer = new StringBuffer();
                                String songid="";
                                while (ci.moveToNext()) {
                                    buffer.append("name : " + ci.getString(0));
                                   songid = ci.getString(0);
                                }
                                Toast.makeText(getApplicationContext(), (String) buffer.toString(),
                                        Toast.LENGTH_LONG).show();
                                db.execSQL("INSERT INTO tagged VALUES('" + songid + "','" + name + "','" + value + "');");
